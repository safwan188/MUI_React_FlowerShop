...
import CurrentOrderStatus from '../../Customer/CustomerProfile/CurrentOrderStatus';
          ...
          Date: {new Date(order.createdAt.seconds * 1000).toLocaleDateString("en-US")}
            ...
            <CardActions>
              {order.orderStatus !== 'delivered' && (
                <Button size="small"     variant="contained" color="primary" onClick={() => handleChangeStatus(order.id)}>
                  {order.orderStatus === 'paid' ? 'prepare' : 'deliver'}
                </Button>
              )}
              {/* Add Give Coupon Button here */}
                {order.orderStatus === 'delivered' && (
         <Button size="small"     variant="contained" color="primary" onClick={() => handleOpen(order.id)}>
                  Give Coupon
                </Button>
              )}
